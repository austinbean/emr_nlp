...
* Regress on *vector* of embeddings.
	...
	hist pred_error if pred_error < `top', title("Prediction Error for 22 Dimensional Prediction") subtitle("Excludes > 99%-ile") graphregion(color(white)) note("X's are a vector of embeddings where the longest " "vector has 22 words")
	...
	hist pred_cons if train == 0, title("Density of Daily Consumption in Oz" "22 Dim. Prediction via Embeddings ") subtitle("Test Data Only") graphregion(color(white)) note("X's are a vector of embeddings where the longest " "vector has 22 words")
